...
#pragma omp parallel for num_threads(16) private (u,compute_it,j,k,iDIM,jDIM,ijSum, ijkSum) reduction(+:aggregate, tempNew) reduction(+:u0,u1,u2,u3,u4,u5,u6,u7,u8,u9)
  for (i=1; i<DIM-1; i++) {
    iDIM=i*DIMSQ;
    for (j=1; j<DIM-1; j++) {
      jDIM=j*DIM;
